...
class NewsScrapping():
    ...
    def __init__(self, query, token):
        self.query = query
        self.webhoseio_query = self.query + " language:(english) (site_type:news)"
    ...
    def start_fetching(self):
        ...
        # n_jobs = 10 since i am fetching 10 articles
        self.final_doc = Parallel(n_jobs=10, backend='threading')(delayed(self.fetch_results_and_store_in_list)(i) for i in inputs)
    ...
    def fetch_results_and_store_in_list(self, i):
        ...
        if redirected_url == "@@FROM_STRING_ERROR@@" or redirected_url == "@@NO_REDIRECTED_URL@@":
            return None
            # continue
...
def Scrape(single_tag_with_token):
    my_query = single_tag_with_token[0]
    token = single_tag_with_token[1]

    my_query = re.sub(r'((?<=[a-z])[A-Z]|(?<!\A)[A-Z](?=[a-z]))', r' \1', my_query)  #to convert camel case to phrase
    start = time.time()
    # my_query = "Jat Protests"
