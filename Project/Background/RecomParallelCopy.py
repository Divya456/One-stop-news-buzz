...
class NewsScrapping():
	...
	def get_results(self):
		...
		for t in ts:
			self.output = webhoseio.query(
				"filterWebData", {"q": self.webhoseio_query,"ts":t})
	...
	def calculate_timestamp(self):
		...
		for i in range(len(days_back)):
			date_days_ago = datetime.now() - timedelta(days=self.days_back[i])
			ts.append(int(time.mktime(date_days_ago.timetuple())) * 1000)
		return ts
