	...
	def sum_basic(update_non_redundency=True):
		def weight(sents, distribution):
			...
			def _weight_sent(sent):
				...
				if len(tokens)!=0:
					for t in tokens:
						#print(distribution.get(t))
						su+=distribution.get(t)
					
					su=float(su)/float(len(tokens))
		...
		while len(s) < 7:
			...
			#summary += " " + highest_weight_sentence+"["+str(ind)+"]"
		...
		# stopwords=["tweet","share","watch","read","also read","share this","email","print","more"]
		# while len(word_tokenize(summary)) < 200:
		# 	weights = weight(sents, pd)
		# 	temp=max(zip(sents, weights, indices), key=itemgetter(1))
		# 	highest_weight_sentence = temp[0]
		# 	lowerSent=highest_weight_sentence.lower()
		# 	flag=False
		# 	for stopword in stopwords:
		# 		##print()
		# 		if stopword in lowerSent:
		# 			flag=True
		# 			break
		# 	if flag is False:
